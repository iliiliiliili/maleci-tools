...
if __name__ == "__main__":
    ...
    if not has_fire:
        ...
        print(f"Fire import already included into {tree.name if len(tree.name) > 0 else tree.path}")
    ...
    if has_name_main_call:
        ...
        for i, node in enumerate(name_main_call_nodes):
            ...
            if len(fire_calls) > 0:
                has_fire_call = True
                print(f"Fire call already included into {tree.name if len(tree.name) > 0 else tree.path} ")
            else:
